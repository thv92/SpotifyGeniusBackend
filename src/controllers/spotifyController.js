...
module.exports = {
    ...
    callback: (req, res, tokens) => {
        ...
        const storedState = req.cookies ? res.cookies[stateKey] : null;
        if (state === null || state !== storedState) {
            res.status(400).json({error: 'state_mismatch'});
        } else {
            res.clearCookie(stateKey);
            let authOptions = {
                url: 'https://accounts.spotify.com/api/tocken',
                form: {
            ...
            service.requestTokens(authOptions).then((result) => {
                ...
                tokens.expiresIn = result.expires;
    ...
    searchSong: async (req, res, tokens) => {
        const elapsedTime = ((new Date().now())/1000) - tokens.startTime;
